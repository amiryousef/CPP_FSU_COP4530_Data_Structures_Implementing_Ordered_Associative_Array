...
namespace fsu
{
  ...
  public:
    ...
    void      Merge     (List<T>& list);  // merges list into *this
    void      Reverse   ();

    template < class Predicate > // P object used to determine order
    void      Sort      (const Predicate& p);

    template < class Predicate > // P object used to determine order
    void      Merge     (List<T>& list, const Predicate& p);
    ...
    Iterator  End        (); // return iterator "passed the back"
    Iterator  rBegin     (); // return iterator to back
    Iterator  rEnd       (); // return iterator "passed the front" in reverse
    Iterator  Includes   (const T& t); // returns position of first occurrence of t
...
[12] Note that list.cpp is a "slave file", that is, it is included inside the
