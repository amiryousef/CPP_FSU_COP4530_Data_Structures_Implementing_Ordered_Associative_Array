...
namespace fsu
{
  ...
  {
    // This implementation is very like that for Dump(os, cw, fill), except that
  ...
  template < typename K , typename D , class P >
  void OAA<K,D,P>::Dump (std::ostream& os, int cw) const
  {
    if (root_ == 0)
      return;
    Queue < Node * , Deque < Node * > > Que;
    ...
    {
      nextLayerSize = 0;
      os << ' '; // indent picture 1 space
      for (j = 0; j < currLayerSize; ++j)
      {
	current = Que.Front();
	Que.Pop();
	os << ColorMap(current->flags_) << std::setw(cw) << current->key_<< ANSI_COLOR_RESET;
	if (current->lchild_ != 0)
	{
	  Que.Push(current->lchild_);
	  ++nextLayerSize;
  ...
  template < typename K , typename D , class P >
  void OAA<K,D,P>::Dump (std::ostream& os, int cw, char fill) const
  {
    if (root_ == 0)
      return;
