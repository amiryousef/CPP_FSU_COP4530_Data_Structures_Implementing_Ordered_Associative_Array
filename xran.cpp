...
namespace fsu
{
  ...
  // uses clock to seed generator, activates to get random fill
  {
    ...
    if (4 != sizeof(unsigned int) || 8 != sizeof(unsigned long))
    {
      std::cerr << " **************************************\n";
      std::cerr << " *  xran warning - size incompatibility:\n"
		<< " *   sizeof(unsigned int)  = " << sizeof(unsigned int) << '\n'
		<< " *   sizeof(unsigned long) = " << sizeof(unsigned long) << '\n'
                << " *  randomness not ensured!\n";
      std::cerr << " **************************************\n";
    }
  ...
  Random_cstring::Random_cstring() : Random_int()
  {}

  char* Random_cstring::Get(int n)
  // returns random (C-style) character string of length n
  {
    if (n < 0)
    {
      std::cerr << "*** xran error: negative length passed to function"
		<< " Random_cstring::Get(int length).";
      exit(1);
    }
    ...
    int i;
    for (i = 0; i < n; ++i)
      str[i] = char(int('a') + Random_int::Get(0,26));
    str[n] = '\0';
    return str;
  }
  ...
  Random_letter::Random_letter() : Random_int()
  {}

  char Random_letter::Get()
  // returns random lower case letter
  {
    return char(int('a') + Random_int::Get(0,26));
  }
